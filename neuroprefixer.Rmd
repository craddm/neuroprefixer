...
bbcHeadlines<-most_read[seq(2,20,2)]
...
googleHeadlines <-  gsub("( -).*","",google_rss[3:length(google_rss)])

#combine them into a single set of headlines
headlines <- c(googleHeadlines,bbcHeadlines)

# This is for removing headlines with words that are likely to produce tasteless headlines; not complete yet, still manually validating headlines
bannedWords <- c("murder","kill(er|ing|ed|)?s","dea(th|d|dly)","genocide","injur(e|i?es|ed|y)","die(s|d)")
headlines <- headlines[grep(paste(bannedWords,collapse="|"),headlines,invert = TRUE)]
...
selectedHeadline <- sample(1:length(headlines),1)
oldHeadline <- headlines[selectedHeadline]
...
#replace the old nouns with the new neuronouns and put them back together to make a new headline
